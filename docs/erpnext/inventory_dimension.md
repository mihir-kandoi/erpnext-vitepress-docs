...
Inventory dimensions in ERPNext are used to track an inventory with multiple parameters. By default, ERPNext allows you to track inventory using warehouses, batches, and serial numbers. If users want to track inventory with custom parameters then they can configure it using the Inventory Dimension feature. The user gets an option to select the Inventory Dimension on respective inventory documents as well in the stock ledger and stock balance report. With this feature, you can view dimension-wise stock ledger and stock balance reports.
...
- Users can filter the Stock Balance and Stock Ledger report using Inventory Dimension
- With this feature, users can see inventory-dimension-wise available quantity
...
Users can only use stock reconciliation with inventory dimensions to enter opening values. They cannot use stock reconciliation to modify the available stock or valuation. Since inventory-dimension-wise valuation rates are not maintained, it does not make sense to allow valuation rates to be modified through stock reconciliation. If users try to update the quantity or valuation rate through stock reconciliation, the system will show the following error:
