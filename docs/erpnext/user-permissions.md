...
When user permissions are defined, for a particular role - the role permissions are only applied when the user permissions are defined. When there are no user permissions for a particular user, there could be two interpretations:
...
To know more, go to the [System Settings page](/erpnext/system-settings).
