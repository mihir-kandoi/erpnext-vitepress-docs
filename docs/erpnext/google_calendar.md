...
- Once Google Calendar Integration is successful, all the events created in ERPNext will be synced if `Push to Google Calendar` is checked.
- Creating an Event in ERPNext
...
- Once Google Calendar Integration is successful, all the events in Google Calendar will be synced if `Pull from Google Calendar` is checked.
