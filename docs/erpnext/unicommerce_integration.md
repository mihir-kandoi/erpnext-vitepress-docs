...
- RTO (return to origin)
  When shipment is returned to origin, shipment status changes and a Credit Note (with Update Stock) is created in ERPNext. Credit Note will be full return.
- CIR (Customer Initiated Return)
  When a customer returns shipment, it is reflected on Unicommerce's Sales Order section. A full or partial Credit Note is created depending upon the nature of customer initiated return.
